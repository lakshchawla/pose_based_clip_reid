"""Vendored from bpbreid's torchreid/models/resnet.py (itself sourced from
https://github.com/pytorch/vision), copied verbatim rather than imported from the external
torchreid package -- unlike bpbreid's HRNet or the BPBreID part-based head, this file has zero
torchreid-specific imports (only torch + torch.utils.model_zoo), so vendoring it introduces no
dependency tree at all. Not currently wired into BPBReIDEncoder or any training script -- BPBreID
still resolves 'resnet50' through torchreid's own internal model registry when used as a backbone
via BPBReIDModelCfg(backbone='resnet50'); this is a separate, standalone copy for direct use
(e.g. a plain, non-part-based baseline), verified independently (see progress.md).
