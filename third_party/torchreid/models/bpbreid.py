...
class BPBreID(nn.Module):
    ...
    def __init__(self, num_classes, pretrained, loss, model_cfg, horizontal_stripes=False, **kwargs):
        ...
        # number of parts K
        self.parts_num = self.model_cfg.masks.parts_num     # 5
        # whether to perform horizontal stripes pooling similar to PCB
        self.horizontal_stripes = horizontal_stripes        # False
        # use shared weights/parameters between each part branch for the identity classsifier
        self.shared_parts_id_classifier = self.model_cfg.shared_parts_id_classifier         # false
        # at test time, perform a 'soft' or 'hard' merging of the learned attention maps with the external part masks
        self.test_use_target_segmentation = self.model_cfg.test_use_target_segmentation     # none
        # use continuous or binary visibility scores at train time:
        self.training_binary_visibility_score = self.model_cfg.training_binary_visibility_score     # true
        # use continuous or binary visibility scores at test time:
        self.testing_binary_visibility_score = self.model_cfg.testing_binary_visibility_score       # true
